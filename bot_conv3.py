...
def event(update, context):
    ...
    logger.info("Harassment_flg %d: ", results.iloc[0]['Harassment_flg'])
    #update.message.reply_text(label)
    if results.iloc[0]['Harassment_flg'] == 2:
        logger.info("Event is harassment")
        Harassment = 'yes'
...
def location(update, context):
    ...
    evento['text'] = evento['text'] + ' ' + update.message.text
    #bert.classifyWithBert(evento['text'][0], logger)
    results = mm.finale(evento['text'], 10, 0.7);
    Date = results.iloc[0]['Date']
    Time = results.iloc[0]['Time']
    Location = results.iloc[0]['Location']
...
def time(update, context):
    ...
    Date = results.iloc[0]['Date']
    Time = results.iloc[0]['Time']
    logger.info("Bot received results:")
    logger.info(results)
    logger.info("Bot received time:")
    logger.info(results.iloc[0]['Time'])
