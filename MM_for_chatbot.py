...
def tfidf(train_data, text, target, min_df, max_df, ngram_range1, ngram_range2): 
    ...
    #transform
    vect_tfidf = TfidfVectorizer(min_df= min_df, max_df= max_df, ngram_range=(ngram_range1, ngram_range2)).fit(train_data2[text].astype(str))
...
#For NER
stanford_ner_tagger = StanfordNERTagger('./stanford_ner/' + 'classifiers/english.muc.7class.distsim.crf.ser.gz','./stanford_ner/' + 'stanford-ner-3.7.0.jar')
...
#Process text *********************************************************************************************
    ...
    #Predictions **************************************************************************************************
        ...
        test_doc2 = test_doc.apply(lambda x: TaggedDocument(words=tokenize_text(x['Corpus']), tags=[x['Target']]), axis=1)
